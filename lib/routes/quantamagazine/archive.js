    ...
    const out = await Promise.all(
        list.map(async (item) => {
            ...
            if (contents !== null) {
                ...
                contents = latex.replace(/<div id=[\S\s]+?"src":"(https?:?[\s\S]+?)",[\S\s]+?"caption":"([\s\S]*?)",[\S\s]+?<\/div>?/g, (omit, src, cap) => {
                    const imgUrl = src.replace(/\\([^un])/g, '$1');
                    const img = '<img src=' + imgUrl + ' />';

                    const noBS = cap.replace(/\\([^un])/g, '$1');
                    const removeNL = noBS.replace(/\\n/g, '');
                    const caption = removeNL.replace(/\\u([0-9]{1,3}[a-z][0-9]?|[0-9]{4}?)/g, (omit, s) => String.fromCharCode(parseInt(s, 16)));
