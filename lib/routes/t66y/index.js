...
module.exports = async (ctx) => {
    ...
    const data = iconv.decode(res.data, 'utf-8');
    ...
    const parseContent = (htmlString) => {
        ...
        for (let k = 0; k < images.length; k++) {
            $(images[k]).replaceWith(`<img src="${$(images[k]).attr('ess-data')}" />`);
        }
        // Handle input tag
        images = $('input');
        for (let k = 0; k < images.length; k++) {
    ...
    const out = await Promise.all(
        list.map(async (item) => {
            ...
            if (cheerio.load(title.html())('font').text() !== '') {
                title = cheerio.load(title.html())('font').text();
            } else {
                title = title.text();
            }
