            ...
            ctx.cache.tryGet(link, async () => {
                ...
                if (encoded) {
                    ...
                    for (const contentPar of rawContent.matchAll(contentPattern)) {
                        const decodedStr = contentPar[0].replace(regex16, (match, grp) => String.fromCharCode(parseInt(grp, 16))).replace(regex8, (match, grp) => String.fromCharCode(parseInt(grp, 8)));
