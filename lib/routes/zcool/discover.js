...
module.exports = async (ctx) => {
    ...
    const currentUrl = `${rootUrl}/discover.json?${query}`;
    ...
    const list = response.data.data.data.map((item) => ({
        ...
        pubDate: new Date(item.object.publishTime),
            ...
            ctx.cache.tryGet(item.link, async () => {
                ...
                if (videos) {
                    content('.video-content-box').each(function (i) {
                        ...
                        content(this).append(`<video src="${videos[i].match(/source: '(https:\/\/video\.zcool\.cn\/.*)'/)[1]}" controls="controls">`);
                    });
                }

                item.description = item.link.includes('article') ? content('.work-content-wrap').html() : content('.article-content-wraper').html();
    ...
    ctx.state.data = {
        title: `${categories[queries.cate]}${queries.subCate === '0' ? '' : ` - ${subCategories[queries.cate][queries.subCate]}`} - 站酷`,
