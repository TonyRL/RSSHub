...
module.exports = async (ctx) => {
    ...
    if (!token) {
        throw 'This RSS feed is disabled due to its incorrect configuration: the token is missing.';
    }
    ...
    function filterHandler(item) {
        ...
        switch (filter) {
            ...
            //     break;
            case 'feed_name':
                if (parseInt(option) === 1 && !setFeedName.length) {
                    addFeedName = 1;
                    setFeedName.push(1);
                }
                item = '';
            ...
            case 'category':
                isNaN(parseInt(option)) ? (item = '') : (item = `category_id=${option}`);
                break;
            case 'order':
    ...
    if (feeds.search(/feed[s]?=/g) !== -1 || !isNaN(parseInt(feeds.split('&').join('')))) {
        const feedsID = feeds.replace(/feed[s]?=/g, '');
        const feedsList = [].concat(feedsID.split('&'));

        if (limit && ctx.query.limit) {
            if (limit < ctx.query.limit) {
                ctx.query.limit = limit * feedsList.length;
            } else {
                const eachLimit = parseInt(ctx.query.limit / feedsList.length);
                if (eachLimit) {
                    limit = eachLimit;
                } else {
                    limit = 1;
