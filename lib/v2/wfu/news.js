...
module.exports = async (ctx) => {
  ...
  const result = await Promise.all(
      ...
      list.map(async (item) => {
            ...
            const $item_url = 'https://www.wfu.edu.cn' +
                              $('a').attr('href');   // 获取 每一项的url
            const $title = $('a>div.txt>h1').text(); // 获取每个的标题
            const $pubdate = timezone(
                parseDate($('a>div.txt>span.date').text().split('：')[1]),
                +8); // 获取发布时间
            ...
            const other = await ctx.cache.tryGet(
                $item_url, async () => await load($item_url));
