...
module.exports = {
  ...
  ProcessItems : async (language, currentUrl, tryGet) => {
    ...
    let items = $('.videotextlist, #video_comments')
                    .find('a')
                    .toArray()
                    .filter((i) => $(i).parent().hasClass('video') ||
                                   $(i).parent().get(0).tagName === 'strong')
                    .map((item) => {
                      item = $(item);

                      const table = item.parentsUntil('table');
                      const link = `${rootUrl}/${language}/${
                          item.attr('href').replace(/^\.\//, '')}`;
        ...
        (item) => tryGet(item.url, async () => {
          ...
          item.description =
              art(path.join(__dirname, 'templates/description.art'), {
                ...
                comment : item.description?.replace(/\[img\]/g, '<img src="')
                              ?.replace(/\[\/img\]/g, '"/>'),
                thumbs : content('.previewthumbs img')
                             .toArray()
                             .map((img) => content(img).attr('src').replace(
                                      /-/g, 'jp-')),
                videos : [...new Set(
                    detailResponse.data.match(/(http[^[\]"]+\.mp4)/g)) ],
              });
          item.pubDate =
              item.pubDate.toString() === 'Invalid Date'
                  ? parseDate(content('#video_date').find('.text').text())
                  : item.pubDate;
