    ...
    const all = await Promise.all(
        types.map(async (type) => {
            const response = await got({
                method: 'get',
                url: `https://docs.rsshub.app/${lang}${type}.html`,
            });
    ...
    ctx.state.data = {
        ...
        item: list.map(({ page, item, type }) => {
            ...
            return {
                ...
                link: `https://docs.rsshub.app/${lang}${type}.html#${encodeURIComponent(titleEle.find('.header-anchor').attr('href') && titleEle.find('.header-anchor').attr('href').slice(1))}`,
                guid: item.attr('id'),
            };
        }),
    };
