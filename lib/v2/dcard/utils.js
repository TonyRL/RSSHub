...
const getCookieJar = async (browser, cache) => {
    let cookieJar = await cache.get('dcard:cookieJar');
    ...
    if (cacheMiss) {
        ...
        logger.debug('Requesting https://www.dcard.tw/f');
        ...
        await cache.set('dcard:cookieJar', cookieJar.toJSON());
    } else {
        logger.debug('Using cached cookieJar');
    }
