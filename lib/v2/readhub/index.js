...
module.exports = async (ctx) => {
  ...
  const apiUrl =
      `${apiRootUrl}/${category === 'daily' ? 'topic/daily' : category}`;
  ...
  const items = await Promise.all(
      list.map((item) => ctx.cache.tryGet(item.id, async () => {
        if (item.hasInstantView || category === 'daily') {
          ...
          item.description =
              `<a href="${data.url}">访问原网址</a><br>${data.content}`;
        }
        ...
        if (isNaN(item.id)) {
          ...
          if (data.newsArray) {
            ...
            for (const news of data.newsArray) {
              item.description +=
                  `<li><a href="${news.url || news.mobileUrl}">${
                      news.title}</a>` +
                          news.siteName
                      ? `&nbsp;&nbsp;<small><b>${news.siteName}</b></small>`
                      : '' +
                            `${
                                news.publishDate ? `&nbsp;&nbsp;<small>${
                                                       news.publishDate.split(
                                                           'T')[0]}</small>`
                                                 : ''}</li>`;
            }
          ...
          if (data.timeline && data.timeline.topics) {
            const hasTimeline = data.timeline.commonEntities &&
                                data.timeline.commonEntities.length > 0;

            item.description +=
                `<br><b>${hasTimeline ? '事件追踪' : '相关事件'}</b><ul>`;

            for (const news of data.timeline.topics) {
              const createdAt =
                  news.createdAt
                      ? `<small>${news.createdAt.split('T')[0]}</small>`
                      : '';

              item.description += `<li>${
                  hasTimeline ? `${createdAt}&nbsp;&nbsp;` : ''}<a href="${
                  rootUrl}/topic/${news.id}">${news.title}</a>${
                  hasTimeline ? '' : `&nbsp;&nbsp;${createdAt}`}</li>`;
            }
