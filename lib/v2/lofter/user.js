...
module.exports = async (ctx) => {
  ...
  const response = await got({
    method : 'post',
    url :
        `http://api.lofter.com/v2.0/blogHomePage.api?product=lofter-iphone-10.0.0`,
    form : {
      blogdomain : rootUrl,
      checkpwd : '1',
      following : '0',
  ...
  const items = response.data.response.posts.map(
      (item) => ({
        ...
        description :
            JSON.parse(item.post.photoLinks || `[]`)
                ...
                JSON.parse(item.post.embed ? `[${item.post.embed}]` : `[]`)
                    .map(
                        (video) => `<video src="${video.originUrl}" poster="${
                            video.video_img_url}" controls="controls"></video>`)
                    .join('') +
                item.post.content,
