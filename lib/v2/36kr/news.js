...
module.exports = async (ctx) => {
  ...
  const currentUrl = `${rootUrl}/${
      links.hasOwnProperty(category) ? links[category]
  ...
  const data = JSON.parse(
      response.data.match(/<script>window\.initialState=(.*?)<\/script>/)[1]);

  const list =
      data.information.informationList.itemList
  ...
  ctx.state.data = {
    title :
        `${response.data.match(/shareTitle: "(.*)频道.*",/)[1]} - 36氪资讯`,
    link : currentUrl,
    item : items,
  };
};
