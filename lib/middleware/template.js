...
module.exports = async (ctx, next) => {
    ...
    const debugKey = ctx.query.debug;

    if (outputType === 'debug.json' && debugInfo) {
        ctx.set({
            'Content-Type': 'application/json; charset=UTF-8',
        });
        if (ctx.state.json && (debugInfo === 'true' || debugInfo === debugKey)) {
            ctx.body = JSON.stringify(ctx.state.json, null, 4);
        } else {
            ctx.body = JSON.stringify({ message: 'plugin does not set debug json' });
        }
    }
    if (outputType.endsWith('.debug.html') && (debugInfo === 'true' || debugInfo === debugKey)) {
        ctx.set({
