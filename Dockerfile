...
RUN apt update && apt install -yq libgconf-2-4 apt-transport-https git dumb-init python3 build-essential --no-install-recommends
...
RUN if [ "$PUPPETEER_SKIP_CHROMIUM_DOWNLOAD" = 0 ]; then \
  apt update \
  && apt install -y wget gnupg ca-certificates --no-install-recommends \
  ...
  && apt purge --auto-remove -y wget gnupg; \
  fi;
