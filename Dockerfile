...
COPY ./scripts/docker/install-deps.sh /app/scripts/docker/
COPY ./yarn.lock /app/
...
RUN \
    ...
    bash scripts/docker/install-deps.sh && \
    yarn cache clean
...
# version unchanged.
# node:lts-bullseye-slim is based on debian:bullseye-slim so this stage would not cause any additional download.
...
FROM node:lts-bullseye-slim as docker-minifier
...
FROM node:lts-bullseye-slim as chromium-downloader
